...
namespace ds::utils
{
    ...
    ListAnalyzer<List>::ListAnalyzer(const std::string& name) :
        ComplexityAnalyzer<List>(name, [this](List& list, size_t size)    //lambda [doplnkove parametre](parametre){kod}
            {
                this->insertNElements(list, size);  //vlozenie dat
            }),
        //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        //01
    ...
    void ListInsertAnalyzer<List>::executeOperation(List& structure)
    {
        ...
        structure.insert(structure.begin(), this->getRandomData());           //vlozenie na zaciatok         begin automaticky ukazuje na zaciatok zoznamu
    ...
    void ListRemoveAnalyzer<List>::executeOperation(List& structure)
    {
        ...
        structure.erase(structure.begin());                             // odstranenie prvkov zo zaciatku
    }
}
