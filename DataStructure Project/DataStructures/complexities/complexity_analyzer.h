...
namespace ds::utils
{
    ...
    {
        // 01
        this->resetSuccess();                                           //boolean nastavenia na false - v ramci GUI
        std::map<size_t, std::vector<duration_t>> data;                 //hashmapa pre ukladanie vysledkov

        for (int r = 1; r <= this->getReplicationCount(); ++r)          //opakovanie testov
        {
            Structure st(structurePrototype);                           //copy constructor
            for (int s = 1; s <= this->getStepCount(); ++s)             //opakovanie s vacsim poctom prvkov
            {
                size_t expectedSize = s * getStepSize();                //pocet prvkov potrebny      size-t - namiesto int na oznacenie velkosti
                size_t count = expectedSize - st.size();                //pocet prvkov pridany
                this->insertN_(st, count);                              //pridanie prvkov do struktury

                this->beforeOperation(st);                              //nie je nutne

                auto start = std::chrono::high_resolution_clock::now(); //zaciatok merania casu      auto - automaticky zavedie var (setrenie dlhych nazvov ONLY)
                this->executeOperation(st);                             //vykona meranie operacie
                auto end = std::chrono::high_resolution_clock::now();   //koniec merania casu

                this->afterOperation(st);                               //nie je nutne

                duration_t duration = std::chrono::duration_cast<duration_t>(end - start);     //ulozenie meraneho casu     duration_t zadefinovany meraci cas
                data[expectedSize].push_back(duration);                 //ulozenie vysledku
            }
        }

        this->saveToCsvFile(data);                                      //ulozenie dat 
        this->setSuccess();                                             //boolean nastavenia na true - v ramci GUI
    }
