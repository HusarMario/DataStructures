...
namespace ds::adt {
    ...
    {
        size_ = 0;
        primaryRegion_->processAllBlocksForward([](typename amt::IS<SynonymTable*>::BlockType* blokSynov)
            {
                delete blokSynov->data_;
    ...
    void GeneralBinarySearchTree<K, T, BlockType>::removeNode(BVSNodeType* node)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            son = this->getHierarchy()->hasLeftSon(*node) ?
            ...
            break;
        case 2:
            BVSNodeType * inOrderPred = this->getHierarchy()->accessLeftSon(*node);
            while (this->getHierarchy()->hasRightSon(*inOrderPred))
            {
                inOrderPred = this->getHierarchy()->accessRightSon(*inOrderPred);
            }
            std::swap(node->data_, inOrderPred->data_);
            this->removeNode(inOrderPred);
            break;
        }
    ...
    void GeneralBinarySearchTree<K, T, BlockType>::rotateLeft(BVSNodeType* node)
    {
        ...
        BVSNodeType* parent = this->getHierarchy()->accessParent(*node);
        BVSNodeType* beforeParent = this->getHierarchy()->accessParent(*parent);

        this->getHierarchy()->changeRightSon(*parent, nullptr);
        this->getHierarchy()->changeLeftSon(*node, nullptr);

        if (beforeParent != nullptr)
        {
            if (beforeParent->left_ == parent)
            {
                this->getHierarchy()->changeLeftSon(*beforeParent, node);
            }
            else
            {
                this->getHierarchy()->changeRightSon(*beforeParent, node);
            }
        }
        else
        {
    ...
    void Treap<K, T>::balanceTree(BVSNodeType* node)
    {
        ...
        node->data_.priority_ = this->rng_.operator()();
        BVSNodeType* parent = this->getHierarchy()->accessParent(*node);
